...
def get_calendar_events():
    ...
    SERVICE_ACCOUNT_FILE = 'credentials/service_account.json'  # <- Path till din JSON
    calendar_id = 'din-kalender-id@group.calendar.google.com'  # <- Kalenderns ID

    creds = service_account.Credentials.from_service_account_file(
        SERVICE_ACCOUNT_FILE, scopes=SCOPES)
    service = build('calendar', 'v3', credentials=creds)

    now = datetime.utcnow().isoformat() + 'Z'  # Idag från nu
    ...
    if not events_today:
        # Om inget för idag, hämta nästa dag
